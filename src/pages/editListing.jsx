...
export default function EditListing() {
  ...
  async function onSubmitHandler(e) {
    ...
    let imageUrl = !checked
      ? await Promise.all([...image].map((image) => storeImage(image))).catch(
          (error) => {
    ...
    const docRef = await updateDoc(
      doc(db, "listings", param.listingId),
      formDataCopy
    );
    setLoading(false);
    navigate(`/profile`);
        ...
        <div className="my-6">
          ...
          <div className="flex items-center text-gray-400">
            <input
              ...
              onChange={onChangeHandler}
              id="checked"
              className="mx-2 my-2 w-5 h-5"
            />
            <p>maintain the previous image</p>
          </div>
        </div>
